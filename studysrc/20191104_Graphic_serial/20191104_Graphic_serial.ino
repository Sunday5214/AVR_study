...
void loop() {
  ...
  LCD_Clear_Serial();
  for(int i = 0; i<8;i++){
    GraphicLCD_Data_Serial(i, 3, 0xaaaa);
    delay(1000);
  }
  delay(1000);
}
...
void GraphicLCD_Data_Serial(int x, int y, unsigned short data){
  GraphicLCD_Serial_INST(0x36);
  GraphicLCD_Serial_INST(0x80+(y%32));
  GraphicLCD_Serial_INST(0x80+(y/32)*8+x);
...
void LCD_Clear_Serial(){
  for(int j = 0; j<8; j++){
    for(int i = 0; i<64; i++){
      GraphicLCD_Serial_INST(0x36); //Extended mode, Graphic display
